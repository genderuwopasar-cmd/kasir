# aplikasi_kasir_complete_fixed.py
...
def barcode_generator_window(parent):
    ...
    ttk.Label(frm, text="Teks untuk Barcode:", font=("Arial", 10, "bold")).pack(anchor="w", pady=(10,5))
    entry_teks = ttk.Entry(frm, width=30, font=("Arial", 10))
    entry_teks.pack(fill="x", pady=5)
    ...
    def print_barcode():
        ...
        try:
            ...
            image = Image.open(win.barcode_path)
            ...
            try:
                ...
                # Hitung scaling agar muat di kertas thermal (70% dari lebar kertas)
                scale_factor = 0.7
                print_width = int(printable_width * scale_factor)
                ...
                # Posisi center
                x = (printable_width - print_width) // 2
                y = 50

                # Print gambar barcode
                dib = ImageWin.Dib(image)
                dib.draw(hdc.GetHandleOutput(), (x, y, x + print_width, y + print_height))

                # Tambahkan teks di bawah barcode
                hdc.SetMapMode(4)
                hdc.TextOut(x, y + print_height + 20, win.barcode_teks)

                # End print job
                hdc.EndPage()
                hdc.EndDoc()

                status_label.config(text=f"✅ Barcode '{win.barcode_teks}' berhasil di-print!")
                messagebox.showinfo("Sukses", "Barcode berhasil dikirim ke printer!")
                sound_mgr.play_sound("success")
    ...
    # Tombol tutup
    ttk.Button(frm, text="Tutup", command=win.destroy).pack(pady=10)
    ...
    entry_teks.bind('<Return>', on_enter)
        ...
        try:
            with zipfile.ZipFile(backup_file, 'w', zipfile.ZIP_DEFLATED) as zipf:
                ...
                if os.path.exists(STRUK_DIR):
                    for root, dirs, files in os.walk(STRUK_DIR):
                        for file in files:
...
def proses_bayar(parent, items, tree, refresh_total):
    ...
    def show_transaksi_sukses(total, bayar, kembali):
        ...
        sukses_window.bind('<Return>', lambda e: tutup_semua())
    ...
    bayar_entry.bind('<Return>', lambda e: auto_bayar())
    ...
    bayar_dialog.bind('<KeyPress>', on_key_press)
...
def main_window():
    ...
    def apply_theme_to_widgets():
        ...
        suggestion_listbox.config(bg=theme["entry_bg"], fg=theme["fg"],
    ...
    main_container.rowconfigure(1, weight=1)  # Treeview area akan expand
    ...
    # Suggestion listbox
    suggestion_listbox = tk.Listbox(main_container, height=8, bg=theme["entry_bg"], fg=theme["fg"],
    ...
    def show_suggestions():
        ...
        if current_suggestions and not suggestion_visible:
            x = kode_entry.winfo_rootx() - root.winfo_rootx()
            y = kode_entry.winfo_rooty() - root.winfo_rooty() + kode_entry.winfo_height()
            suggestion_listbox.place(x=x, y=y, width=kode_entry.winfo_width(), height=200)
            suggestion_visible = True
            suggestion_listbox.selection_set(0)
        elif not current_suggestions:
            hide_suggestions()

    def hide_suggestions():
        nonlocal suggestion_visible
        suggestion_listbox.place_forget()
        suggestion_visible = False

    def on_suggestion_select(event=None):
        if suggestion_listbox.curselection() and current_suggestions:
    ...
    def navigate_suggestions(direction):
        """Navigasi suggestion list dengan arrow keys"""
        if not suggestion_visible or not suggestion_listbox.size():
            return
        ...
        if not current_index:
            ...
            elif direction == 'down' and current_index < suggestion_listbox.size() - 1:
                new_index = current_index + 1
            else:
                return

            suggestion_listbox.selection_clear(0, tk.END)
            suggestion_listbox.selection_set(new_index)
            suggestion_listbox.see(new_index)
    ...
    # Bind events
    kode_entry.bind('<KeyRelease>', lambda e: show_suggestions() if e.keysym not in ['Up', 'Down', 'Return'] else None)
    kode_entry.bind('<Down>', lambda e: navigate_suggestions('down'))
    kode_entry.bind('<Up>', lambda e: navigate_suggestions('up'))
    kode_entry.bind('<Return>', lambda e: on_suggestion_select() if suggestion_visible else tambah_barang())
    ...
    root.bind('<F2>', lambda e: clear_and_focus_search())

    # Bind angka untuk quick search
    root.bind('<F3>', lambda e: quick_search_by_code("BRG"))
    root.bind('<F4>', lambda e: quick_search_by_code("OLI"))
    root.bind('<F5>', lambda e: quick_search_by_code("BAN"))
    ...
    ttk.Button(button_frame, text="🔍 BARANG (F2)",
               command=clear_and_focus_search).grid(row=0, column=2, padx=2, sticky='ew')

    ttk.Button(button_frame, text="📦 SEMUA",
               command=lambda: quick_search_by_code("")).grid(row=0, column=3, padx=2, sticky='ew')

    ttk.Button(button_frame, text="⛽ OLI",
               command=lambda: quick_search_by_code("OLI")).grid(row=0, column=4, padx=2, sticky='ew')

    ttk.Button(button_frame, text="💰 PROSES PEMBAYARAN (END)",
               command=lambda: proses_bayar(root, items, tree, refresh_total),
               style="Accent.TButton").grid(row=0, column=5, padx=2, sticky='ew')

    # Total frame
    total_frame = ttk.Frame(main_container, padding=10)
    total_frame.grid(row=4, column=0, sticky='ew', pady=(0, 10))
    ...
    def on_resize(event):
        if suggestion_visible:
            x = kode_entry.winfo_rootx() - root.winfo_rootx()
            y = kode_entry.winfo_rooty() - root.winfo_rooty() + kode_entry.winfo_height()
            suggestion_listbox.place(x=x, y=y, width=kode_entry.winfo_width())

    root.bind('<Configure>', on_resize)

    # Bind Enter key untuk tambah barang
    qty_entry.bind('<Return>', lambda e: tambah_barang())

    # Bind End key untuk proses pembayaran
    root.bind('<End>', lambda e: proses_bayar(root, items, tree, refresh_total))

    # Click outside to hide suggestions
    def on_click_outside(event):
        if event.widget not in [kode_entry, suggestion_listbox]:
            hide_suggestions()
...
def kelola_stok_window(parent, on_stok_changed=None):
    ...
    def edit_stok():
        ...
        kode = tree.item(selected[0])['values'][0]
        if kode not in stok:
...
def laporan_window(parent):
    ...
    tree.bind("<Double-1>", lambda e: print_selected_transaksi())
