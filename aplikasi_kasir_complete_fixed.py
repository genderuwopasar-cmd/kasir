...
def bind_fullscreen_keys(win, escape_exits_app=False):
    ...
    try:
        ...
        win.bind("<F12>", lambda e, w=win: toggle_fullscreen(w), add="+")
        if escape_exits_app:
            # Escape should ask to exit the whole app (confirm)
            win.bind("<Escape>", lambda e, w=win: confirm_exit_app(w), add="+")
        else:
            # Escape should at least exit fullscreen for that specific window
            win.bind("<Escape>", lambda e, w=win: exit_fullscreen(w), add="+")
    except Exception:
        pass
...
def main_window():
    ...
    def create_suggestion_popup():
        ...
        suggestion_listbox.bind('<Escape>', lambda e: hide_suggestions())
        suggestion_listbox.bind('<Up>', lambda e: navigate_suggestions('up'))
        suggestion_listbox.bind('<Down>', lambda e: navigate_suggestions('down'))
    ...
    kode_entry.bind('<KeyRelease>', lambda e: show_suggestions() if e.keysym not in ['Up', 'Down', 'Return', 'Escape'] else None)
    kode_entry.bind('<Down>', lambda e: navigate_suggestions('down') or (suggestion_listbox and suggestion_listbox.focus_set()))
    kode_entry.bind('<Up>', lambda e: navigate_suggestions('up') or (suggestion_listbox and suggestion_listbox.focus_set()))
    kode_entry.bind('<Return>', lambda e: on_suggestion_select() if suggestion_visible else tambah_barang())
    kode_entry.bind('<Escape>', lambda e: hide_suggestions())
